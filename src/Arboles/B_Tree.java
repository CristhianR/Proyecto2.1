...
public class B_Tree {
	...
	B_Node root;  //every tree has at least a root node
...
// --------------------------------------------------------
	...
	public B_Node search(B_Node root, int key)
	{
...
//  --------------------------------------------------------
	...
	public void split(B_Node x, int i, B_Node y)
	{
		B_Node z = new B_Node(order,null);//gotta have extra node if we are
	                					//to split.
...
// ----------------------------------------------------------
	...
	public void nonfullInsert(B_Node x, int key, Object val)
	{
		...
		//	i++;
			...
			nonfullInsert(x.child[j],key, val);//recurse
		}
	}
//--------------------------------------------------------------
...
//--------------------------------------------------------------
	...
	public void insert(B_Tree t, int key, Object val)
	{
		B_Node r = t.root;//this method finds the node to be inserted as 
				 //it goes through this starting at root node.
		if(r.count == 2*order - 1)//if is full
		{
			B_Node s = new B_Node(order,null);//new node
			s.val = val;
			...
			nonfullInsert(s, key, val); //call insert method
		}
		else
...
// ------------------------------------------------------------
	...
	public void SearchPrintNode( B_Tree T,int x)
	{
		B_Node temp= new B_Node(order,null);
...
//--------------------------------------------------------------
       ...
       public void deleteKey(B_Tree t, int key)
       {
			       
		B_Node temp = new B_Node(order,null);//temp Bnode
